...
runClassifier <- function(trainData,trainClass,testData,testClass,kInner,defaultTuning,seed, i_kOuter,i_rep){
  ...
  modelFit <- train(x = trainData, y = trainClass, method = 'glmnet', maxit = 1000000, trControl = fitControl, metric = 'ROC')
...
for (i_rep in minRep:maxRep) {
  ...
  outputFilename = paste(timeLabel,'_model_fits__rep_',toString(i_rep),'_dataset',"lungstereo", sep = '')
  ...
  outputFilenameOutputTable = paste(timeLabel,'_models_output_table_rep_-','-',toString(i_rep), sep = '')
}
