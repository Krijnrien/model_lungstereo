...
#data$cT_stat <- ifelse(data$cT != 0 | data$cT != 1, NA, data$cT)
#data$cN_stat <- ifelse(data$cN != 0 | data$cN != 1, NA, data$cN)
#data$cM_stat <- ifelse(data$cM != 0 | data$cM != 1, NA, data$cM)
...
toNumericals = c('survivalstat', 'FEV1', 'size','cum_dosis_specificatie', 'agediag', 'diff_in_days',
